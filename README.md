...
There are three **input** arguments for input data: 1) external_scanner_image_adrs, 2) target_scanner_image_adrs, 2) CVfolds_adrs. 1) _external_scanner_image_adrs_ is the address for images of the _source_ scanner (source data). 2) _target_scanner_image_adrs_ is the address to the directory of images for the source scanners (multi-scanner data). 3) _CVfolds_adrs_ is the address to the list of cross-validated images, if we used cross-fold validation for the multi-scanner data.
```
